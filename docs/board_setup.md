...
Intel Keembay boards have a recovery mode that can be accessed by setting the boot switch to the recovery position. The exact location and configuration of the boot switch varies by board model, so refer to your board's documentation for specific instructions.

1. Power off the board
2. Set the boot switch to the recovery position
3. Connect the USB cable to the recovery port (usually labeled as such on the board)
4. Power on the board

After powering on, the board should be detected as a USB device with VID:PID `8087:0b39`. Once the FIP is flashed, the board will reboot into fastboot mode with VID:PID `8087:da00`.
